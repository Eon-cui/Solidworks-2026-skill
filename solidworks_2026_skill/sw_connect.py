...
def find_template(sw, doc_type="part"):
    ...
    # swDefaultTemplatePart/Assembly/Drawing = 8/9/10; 部分版本 24/25/26 为模板目录。
    # 注册表可能为空或幽灵路径 (指向已删文件) → 全部 os.path 校验 + glob 兜底。
    pref_keys = {"part": (8, 24), "assembly": (9, 25), "drawing": (10, 26)}[doc_type]
    pattern = {"part": "*.prtdot", "assembly": "*.asmdot", "drawing": "*.drwdot"}[doc_type]
    default_path = ""
    for key in pref_keys:
        try:
            default_path = sw.GetUserPreferenceStringValue(key)
        except Exception:
            default_path = ""
        if default_path:
            break
