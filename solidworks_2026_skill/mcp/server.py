...
def _get_model():
    ...
    model = sw.ActiveDoc
    if model is None:
        raise RuntimeError("No open document. Call sw_new_part() or sw_open_doc() first")
    return model
...
async def sw_preflight() -> str:
    ...
    lines = []
    if not HAS_COM:
        lines.append("❌ pywin32 not installed → pip install pywin32")
    else:
    ...
    if HAS_NUMPY:
        ...
        lines.append("⚠️ numpy not installed (gear/polygon features degraded)")
    ...
    if HAS_COM:
        try:
            ...
            try:
                ...
                lines.append(f"❌ SolidWorks unavailable: {str(e)[:80]}")
    return "\n".join(lines)


@mcp.tool()
async def sw_health() -> str:
    """连接健康检查：COM 存活 + 活动Created new  + features数。"""
    try:
        sw = _get_sw()
        ...
        if model:
            ...
            lines.append(f"Created new : {title} @ {path}")
            feat = model.FirstFeature()
            count = 0
            while feat is not None:
                count += 1
                feat = feat.GetNextFeature()
            lines.append(f"features数: {count}")
        else:
            lines.append("Created new : 无")
        return "\n".join(lines)
    except Exception as e:
...
async def sw_connect(visible: bool = True) -> str:
    ...
    try:
        ...
        rev = _com_attr(sw, "RevisionNumber")
        return f"Connected to SW {rev} (PID={pid})"
    except Exception as e:
...
async def sw_instance_info() -> str:
    ...
    return f"SW {rev} | PID={pid} | visible={vis} | doc={title}"


@mcp.tool()
async def sw_disconnect() -> str:
    """Disconnect COM 连接（不关闭 SW 窗口）。"""
    global sw_app
    sw_app = None
    return "已Disconnect"
...
# ═══════════════════════════════════════════════════════════════════
# Category 3: Created new 操作 (15 tools)
# ═══════════════════════════════════════════════════════════════════
...
async def sw_new_part(template: str = "") -> str:
    ...
    if not template:
        ...
        if not template:
            return "Part template not found. Specify template path manually"
    ...
    if doc is None:
        return f"New failed (模板={template})，SW 未返回活动Created new "
    return f"New: {_com_attr(doc, 'GetTitle')}"
...
async def sw_open_doc(filepath: str) -> str:
    ...
    if doc is None:
        return f"Open failed (err={errors.value} warn={warnings.value}): {abs_path}"
    return f"Opened: {_com_attr(doc, 'GetTitle')}"
...
async def sw_save() -> str:
    ...
    return "Save succeeded" if ret else f"Save failed (err={errors.value})"
...
async def sw_save_as(filepath: str) -> str:
    ...
    return f"Saved: {abs_path}" if ret else f"Save failed (err={errors.value}): {abs_path}"
...
async def sw_export_step(filepath: str) -> str:
    ...
    return f"STEP: {abs_path}" if ret else f"Export failed (err={errors.value})"
...
async def sw_export_stl(filepath: str) -> str:
    ...
    warnings = _byref_int(0)
    ret = _safe_com_call(model.Extension.SaveAs, abs_path, _vi4(0), _vi4(1), _vn(), errors, warnings)
    return f"STL: {abs_path}" if ret else f"Export failed (err={errors.value})"
...
async def sw_export_dxf(filepath: str) -> str:
    ...
    warnings = _byref_int(0)
    ret = _safe_com_call(model.Extension.SaveAs, abs_path, _vi4(0), _vi4(1), _vn(), errors, warnings)
    return f"DXF: {abs_path}" if ret else f"Export failed (err={errors.value})"
...
async def sw_export_pdf(filepath: str) -> str:
    ...
    warnings = _byref_int(0)
    ret = _safe_com_call(model.Extension.SaveAs, abs_path, _vi4(0), _vi4(1), _vn(), errors, warnings)
    return f"PDF: {abs_path}" if ret else f"Export failed (err={errors.value})"
...
async def sw_sketch_create(plane: str = "Front Plane") -> str:
    ...
    if not matched:
        return f"Plane not found: {plane}"
    _sketch_mgr().InsertSketch(True)
    return f"Sketch: {matched}"
...
async def sw_sketch_center_rectangle(cx: float, cy: float, w: float, h: float) -> str:
    ...
    return f"Rectangle: 中心({cx},{cy}) {w}×{h}"
...
async def sw_sketch_polygon(cx: float, cy: float, r: float, sides: int = 6) -> str:
    ...
    return f"Regular {sides}-gon: R={r}"
...
async def sw_sketch_centerline(x1: float, y1: float, x2: float, y2: float) -> str:
    ...
    return f"Centerline: ({x1},{y1})→({x2},{y2})"
...
# ═══════════════════════════════════════════════════════════════════
# Category 5: features操作 (20 tools)
# ═══════════════════════════════════════════════════════════════════
...
async def sw_feature_extrude_cut(depth: float, draft: float = 0.0, through_all: bool = False) -> str:
    ...
    if feat is None:
        return f"❌ Cut failed: FeatureCut3 returned None (depth={depth}, thru={through_all}) — check if sketch is closed/selected"
    return f"✅ Cut: depth={depth} thru={through_all}"
...
async def sw_feature_shell(thickness: float = 0.001) -> str:
    """Shell: thickness=壁厚(m)。"""
    fm = _feature_mgr()
    _safe_com_call(fm.FeatureShell, thickness, 1)
    return f"Shell: {thickness}"
...
async def sw_feature_hole_diameter(diameter: float = 0.005, depth: float = 0.01) -> str:
    ...
    return f"Hole: dia {diameter}×{depth}"
...
async def sw_feature_thread(diameter: float = 0.006, pitch: float = 0.001, depth: float = 0.01) -> str:
    ...
    return f"Thread: M{diameter*1000:.0f}×{pitch*1000:.1f}"
...
async def sw_feature_boss_thin(depth: float = 0.005, thickness: float = 0.002) -> str:
    ...
    return f"Thin wall: depth={depth} t={thickness}"
...
async def sw_select_plane(plane_name: str) -> str:
    ...
    return f"Selected: {plane_name}" if ok else f"Not found: {plane_name}"
...
async def sw_select_feature(feature_name: str) -> str:
    ...
    return f"Selected: {feature_name}" if ok else f"Not found: {feature_name}"
...
async def sw_select_vertex(x: float, y: float, z: float) -> str:
    ...
    return f"Vertex selected({x},{y},{z})" if ok else "未找到"
...
async def sw_create_plane_offset(offset: float = 0.01, reference: str = "Front Plane") -> str:
    ...
    if matched:
        ...
        return f"Offset plane: {matched}+{offset}"
...
async def sw_insert_axis(type_: int = 1) -> str:
    ...
    return f"Reference axis type={type_}"
...
async def sw_set_material(material: str = "6061 铝合金") -> str:
    """设置Material: 6061/普通碳钢/304不锈钢/ABS。"""
...
async def sw_set_transparency(transparent: bool = True) -> str:
    ...
    return f"Transparent: {transparent}"
...
async def sw_get_mass_properties() -> str:
    ...
    try:
        ...
        if bb and len(bb) >= 6:
            ...
            return f"Bounding box={dx:.1f}×{dy:.1f}×{dz:.1f}mm | vol≈{vol_mm3:.0f}mm³ | mass≈{est_mass:.1f}g (alu est.)"
    except Exception:
        traceback.print_exc()
...
async def sw_review_quick() -> str:
    """快速审查：等轴测视图 + 包围盒 + features数 + 质量。"""
    model = _get_model()
    lines = []
    # title
    lines.append(f"Created new : {_com_attr(model, 'GetTitle')}")
    # isometric
    ...
    try:
        ...
        lines.append("features数: N/A (SW2026 COM 限制)")
        count = -1
    # mass
    try:
    ...
    if isinstance(count, int) and count > 1:
        lines.append("✅ 有实体features")
    elif count == -1:
        lines.append("✅ 模型已生成（features遍历 COM 不可用，属于已知 SW2026 限制）")
    else:
        lines.append("⚠️ Further inspection needed")
    return "\n".join(lines)
...
async def sw_review_check_blank() -> str:
    ...
    try:
        ...
        while feat is not None:
            ...
            feat = feat.GetNextFeature()
        if not issues:
            return f"✅ No issues (共 {count} 个features)"
        return f"共 {count} features\n" + "\n".join(issues)
    except Exception:
        return "⚠️ features遍历不可用 (SW2026 Python COM 已知限制)，建议在 SW GUI 中目视检查"
...
async def sw_review_thickness_check(min_thickness: float = 0.0005) -> str:
    ...
    return f"{status} Min bounding box dim={min_dim*1000:.2f}mm (threshold={min_thickness*1000:.2f}mm)"
...
async def sw_review_export_screenshot(filepath: str) -> str:
    ...
    return f"Screenshot: {abs_path}" if ret else f"Failed (err={errors.value})"
...
async def sw_gear_create(joint: str = "J1", which: str = "both", segments: int = 30) -> str:
    ...
    if joint not in GEAR_PRESETS:
        return f"Joint {joint} not in presets。Options: {', '.join(GEAR_PRESETS.keys())}"
...
async def sw_gear_verify(joint: str = "all") -> str:
    ...
    for j in valid:
        if j not in GEAR_PRESETS:
            lines.append(f"{j}: not in presets")
            continue
        p = GEAR_PRESETS[j]
...
async def sw_close_all_docs(save: bool = False) -> str:
    """关闭所有打开的Created new 。铁律: 建模任务开始前必须调用清场。

    save=False 不保存直接关 (默认)。
    """
    sw = _get_sw()
    ok = sw.CloseAllDocuments(not save)
    return f"✅ All documents closed" if ok else "⚠️ CloseAllDocuments returned False"


@mcp.tool()
async def sw_quit_doc(title: str = "") -> str:
    """关闭指定标题Created new  (不保存)。title 空 = 当前活动Created new 。
...
async def sw_count_faces() -> str:
    """统计当前零件所有 body 的面数。建模后验证features是否真实生效。

    例: 20×20×10 盒 = 6 面; +1 通孔 = 7 面。
    切除调用"成功"但面数没变 = features没生效 = 假成功!
    """
    model = _get_model()
...
async def sw_verify_step_geometry(step_path: str, expected_holes: str = "") -> str:
    """解析 STEP 文件验证几何features (防假成功核心工具)。
    ...
    if not os.path.exists(step_path):
        return f"❌ File not found: {step_path}"

    file_size = os.path.getsize(step_path)
    if file_size > MAX_STEP_SIZE:
        return f"❌ STEP file too large: {file_size / (1024*1024):.1f}MB (max {MAX_STEP_SIZE / (1024*1024):.0f}MB)"
...
async def sw_asm_add_component_posed(path: str, transform16: str) -> str:
    ...
    实测 (2026-06-12): AddComponent5 只给平移; 旋转必须 Transform2 PUTREF:
      - MathUtility/Component2 dynamic 解析Failed → gen_py 手动包装
    ...
    if len(arr) != 16:
        return "❌ transform16 must have 16 elements"
    sw = _get_sw()
    asm = sw.ActiveDoc
    if asm is None:
    ...
    return f"✅ Component inserted and positioned: {os.path.basename(path)}"
...
async def sw_asm_verify_poses(step_path: str, expected_json: str, tol_mm: float = 0.1) -> str:
    ...
    for item in expects:
        ...
        n_match += found
        if not found:
            lines.append(f"❌ {label}: t={t} z={z} not matched")
    lines.append(f"Pose match {n_match}/{len(expects)} " + ("✅" if n_match == len(expects) else "❌"))
    return "\n".join(lines)
