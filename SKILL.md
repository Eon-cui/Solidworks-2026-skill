...
**写任何 `with SW()` 之前先写 brief。** brief = 零件名/坐标系/尺寸/特征/验证目标/假设的结构化草稿。详见 `references/sw-brief.md`。

## 装配设计账本

**写 `add_mates.py` 前先填五账本。** Robot Metadata / Link Ledger / Joint Ledger / Mating Datum Ledger / Assumption Ledger。每个 mate 对的面选择策略必须在账本中显式化。详见 `references/assembly-design-ledger.md`。
...
| 验收（STEP 几何/面数/装配位姿/7D3S） | `solidworks_2026_skill/sw_verify.py` | `references/verification.md` |
| 跨零件接口交叉校验 | `solidworks_2026_skill/sw_check_interfaces.py` | `references/verification.md` |
| 导出 STEP/STL/PDF/DXF | `solidworks_2026_skill/sw_export.py` | — |
| VBA 宏防护（大模型生成宏时） | `solidworks_2026_skill/sw_macro_guard.py` | — |
| 工程图/运动算例/审查/外观 ⚠未实测 | `solidworks_2026_skill/_untested/` | `references/upstream/` |
...
| 系统化修复协议 | — | `references/troubleshooting.md` H 节 |
| SW Brief（建模前规划） | — | `references/sw-brief.md` |
| 装配设计账本（配合前规划） | — | `references/assembly-design-ledger.md` |
| 快照复核（SaveBMP+Pillow） | `solidworks_2026_skill/sw_snapshot.py` | `references/snapshot-review.md` |
| 选择器引用（STEP→#h1 标签） | `solidworks_2026_skill/sw_selector_refs.py` | `references/selector-reference-system.md` |
| step.parts 外购件集成 | `solidworks_2026_skill/sw_step_parts.py` | `references/step-parts-integration.md` |
...
| 批量建模/装配/验收脚本（可重复执行的 build 脚本） | **直接 import solidworks_2026_skill/**。一个 Python 进程跑完全部操作，避免多进程多 SW 实例 |
...
7. SW Brief 写了？（`references/sw-brief.md`）
8. 装配设计账本填了？（`references/assembly-design-ledger.md`）
9. 建模后快照拍了？（`references/snapshot-review.md` — SaveBMP+Pillow）
10. 标准件先查 step.parts？（`references/step-parts-integration.md`）
