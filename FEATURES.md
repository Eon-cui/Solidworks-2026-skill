# FEATURES — solidworks-2026-skill 能力清单
...
**13 种定制零件 (35+ 零件文件) + 32 实例装配体 + 43 个配合**，期间踩完 27 条坑并全部破解沉淀。
本 Skill = 自建公共库（swlib）+ 自建 mate 自动化 + 自建验收体系 + 121-tool MCP server
+ 全模块 4 对 1 去重合并的大一统封装。

## 能做什么

| 能力 | 入口 | 验证强度 |
|------|------|----------|
| 零件建模（草图/拉伸/切除/圆角/阵列） | `solidworks_2026_skill/sw_session.py` (毫米) / `solidworks_2026_skill/sw_part.py` (米) | 35+ 零件实战，全部 STEP 几何级 PASS |
...
| 配合自动化（同心/贴合/距离/齿轮/LOCK/固定） | `solidworks_2026_skill/sw_mate.py` 程序化面选择 | 43 配合全成功 + 复验零漂移 |
...
| 跨零件接口校验 | `solidworks_2026_skill/sw_check_interfaces.py` | 33 项配对实战 |
| 导出 STEP/STL/IGES/PDF/DXF | `solidworks_2026_skill/sw_export.py` | Extension.SaveAs 实测 |
| 环境自检 | `solidworks_2026_skill/sw_preflight.py` | 依赖+SW 安装+COM 注册 |
| VBA 宏防护 | `solidworks_2026_skill/sw_macro_guard.py` | 模型分流+模板兜底+校验 |
...
- 工程图/运动算例/审查/外观：上游脚本已收录（`solidworks_2026_skill/_untested/` + `references/upstream/`）但**未经本项目实测**，用前验签名
...
- 2026-06-29 v1.2 — English-only runtime + face resilience + connect dedup + 全面交叉审计
- 2026-06-29 v1.1 — _com_helpers/_com_signatures 去重 + MCP wheel 化 + 路径安全加固
- 2026-06-29 v1.0.1 — Phase 1 P0 五颗雷清零
- 2026-06-13 v1.0 — 开源发布：包重组 + 安全加固 + MCP annotations + _untested/ 隔离（UR-SEU-2026 V9 交付后封装）
