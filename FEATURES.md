...
| 能力 | 入口 | 验证强度 |
|------|------|----------|
| 零件建模（草图/拉伸/切除/圆角/阵列） | `sw_2026_skill/sw_session.py` (毫米) / `sw_2026_skill/sw_part.py` (米) | 35+ 零件实战，全部 STEP 几何级 PASS |
...
| 配合自动化（同心/贴合/距离/齿轮/LOCK/固定） | `sw_2026_skill/sw_mate.py` 程序化面选择 | 43 配合全成功 + 复验零漂移 |
...
| 跨零件接口校验 | `sw_2026_skill/sw_check_interfaces.py` | 33 项配对实战 |
| 导出 STEP/STL/IGES/PDF/DXF | `sw_2026_skill/sw_export.py` | Extension.SaveAs 实测 |
| 环境自检 | `sw_2026_skill/sw_preflight.py` | 依赖+SW 安装+COM 注册 |
| VBA 宏防护 | `sw_2026_skill/sw_macro_guard.py` | 模型分流+模板兜底+校验 |
...
- 工程图/运动算例/审查/外观：上游脚本已收录（`sw_2026_skill/_untested/` + `references/upstream/`）但**未经本项目实测**，用前验签名
